          ...
          {keyDates.map((item, index) => (
            ...
            >
              <div className="flex items-center space-x-3 sm:space-x-4">
                <div className={`${item.iconColor} bg-white p-3 sm:p-4 rounded-xl shadow-sm group-hover:shadow-md transition-shadow duration-300 flex-shrink-0`}>
                  <div className="w-5 h-5 sm:w-6 sm:h-6">
                    {item.icon}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className={`text-xs sm:text-sm font-semibold ${item.textColor} uppercase tracking-wide mb-1 sm:mb-2`}>
                    {item.label}
                  </div>
